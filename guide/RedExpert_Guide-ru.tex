...
После этого откроется панель профайлера, состоящая из выпадающего списка для выбора соединения, набора кнопок для управления сессией профайлера, поля "Компактное отображение" и результирующей таблицы.

\begin{figure}[H]
	\centering
	\includegraphics[width = 1\linewidth]{img/profiler_module.png}
...
\ttt{Компактное отображение} меняет способ отображения данных. Если включено, то повторяющиеся процессы внутри общего родительского будут объединены в один. По умолчанию включено.

 \begin{figure}[H]
 	\centering
 	\includegraphics[width = 1\linewidth]{img/profiler_compact_data_display.png}
...
\begin{itemize}
	...
	\item Затраченное на процесс время (с учетом дочерних процессов) и процент времени от родительского процесса;
	\item Среднее затраченное на процесс время (с учетом дочерних процессов) для повторяющихся процессов, объединенных в один узел; 
	\item Количество вызовов повторяющихся процессов.
\end{itemize}
Для каждого не последнего узла (за исключением \ttt{ROOT NODE} - корневого узла) есть узел \ttt{Собственное время}, показывающий затраченное время без учета дочерних процессов.
...
При наведении на ячейку в всплывающей подсказке будет показана сумма значений столбца и среднее значение. Для просмотра полного текста запроса и его плана нужно кликнуть на ячейку правой кнопкой мыши. При двойном клике откроется окно с записью события в текстовом формате.
...
\begin{longtable}[r]{|>{\ttfamily\centering}m{1.8cm}|m{11.7cm}|m{1.8cm}|}
	...
	S & миллисекунды с округлением до десятых & 1 \\\hline
	SS & миллисекунды с округлением до сотых & 11 \\\hline
