...
\begin{itemize}
	...
	\item Выполнение и отображение нескольких запросов с множеством результатов (Result Set);
	...
	\item Поисковая исполняемая история SQL-запросов.
\end{itemize}
...
\anonsubsection{История запросов}
...
На боковой панели \textit{Подключения} отображены имена всех ранее устанавливаемых соединений. После подключения к базе данных на этой панели появляется структура дерева, узлы которого представляют собой объекты базы данных:

\begin{longtable}[c]{|m{5mm}|m{6cm}|m{5mm}|m{8cm}|}
	\hline
	\begin{tikzpicture}
	\pgftext{\includegraphics[width=12pt]{img/TableIndex16.png}} at (0pt,0pt)
	\end{tikzpicture} & ИНДЕКСЫ & 
	\begin{tikzpicture}
	\pgftext{\includegraphics[width=12pt]{img/exception16.png}} at (0pt,0pt)
	\end{tikzpicture} & ИСКЛЮЧЕНИЯ \\\hline
	\begin{tikzpicture}
	\pgftext{\includegraphics[width=12pt]{img/Procedure16.png}} at (0pt,0pt)
	\end{tikzpicture} & ПРОЦЕДУРЫ & 
	\begin{tikzpicture}
	\pgftext{\includegraphics[width=12pt]{img/udf16.png}} at (0pt,0pt)
	\end{tikzpicture} & ВНЕШНИЕ ФУНКЦИИ \\\hline
	\begin{tikzpicture}
...
Двойной клик по узлу откроет вкладку на главной панели с детальным описанием каждого объекта. Например, двойной клик по узлу с названием конкретной процедуры отобразит таблицу с ее входными и выходными параметрами, их типами, тело процедуры и SQL команду создания этой процедуры.
...
Дополнительную информацию см. в разделе \hyperref[sec:connections]{«Подключение к базе данных»}.
...
В этом разделе настроек можно установить плагин с новой темой приложения. И в дальнейшем активировать ее в разделе \textit{Display}.
