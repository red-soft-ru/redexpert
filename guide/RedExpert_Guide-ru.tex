...
Графическая визуализация схемы базы данных.
	\begin{figure}[H]
		\centering
		\includegraphics[width = 0.8\linewidth]{img/erd_employee.png}
	\end{figure}
...
\begin{itemize}
\item \ttt{Компактное отображение} -- Отображает общую картину выполнения запросов. .Повторяющиеся процессы внутри общего родительского будут объединены в один. Является значением по умолчанию.
 \begin{figure}[H]
	\centering
	\includegraphics[width = 0.8\linewidth]{img/profiler_compact_data_display.png}
	\caption{Компактное отображение данных}
\end{figure}
\item \ttt{Расширенное отображение} -- Отображает детализированную картину выполнения запросов. Отоюражается статискика по выполнению каждой строки \ttt{PSQL} кода, а повторяющиеся операции не объединяются.
 \begin{figure}[H]
	\centering
...
\begin{figure}[H]
	\centering
	\includegraphics[width = 1\linewidth]{img/erd_employee.png}
	\caption{ER-диаграмма базы данных Employee}
\end{figure}
...
\begin{longtable}[c]{|m{5mm}|m{9cm}|>{\ttfamily}m{4cm}|}
	...
	\end{tikzpicture} & Удалить выбранные таблицы с диаграммы & Delete\\\hline
	\begin{tikzpicture}
	\pgftext{\includegraphics[width=11pt]{img/CreateScripts16.png}} at (0pt,0pt)
	\end{tikzpicture} & Создать сценарий SQL CREATE TABLE & \\\hline
	\begin{tikzpicture}
	...
	\begin{tikzpicture}
	\pgftext{\includegraphics[width=11pt]{img/AddComment16.png}} at (0pt,0pt)
	\end{tikzpicture} & Добавить текстовый блок & \\\hline
	\begin{tikzpicture}
	...
	\end{tikzpicture} & Изменить цвет фона выбранных таблиц & \\\hline
	\begin{tikzpicture}
	...
	\end{tikzpicture} & Уменьшить размер & Ctrl + колесо мыши\\\hline
	\begin{tikzpicture}
	\pgftext{\includegraphics[width=11pt]{img/ZoomIn16.png}} at (0pt,0pt)
	\end{tikzpicture} & Увеличить размер &  Ctrl + колесо мыши\\\hline
	\begin{tikzpicture}
		\pgftext{\includegraphics[width=11pt]{img/RecycleConnection16.png}} at (0pt,0pt)
	\end{tikzpicture}& Реверс-инжиниринг & \\\hline
\end{longtable}
...
\anonsubsubsection{Обновление диаграммы из базы данных}

В случае изменения структуры базы данных ER-диаграмму необходимо обновить. Для этого нужно нажать на кнопку \begin{tikzpicture}
	\pgftext{\includegraphics[width=11pt]{img/RecycleConnection16.png}} at (0pt,0pt)
\end{tikzpicture} и в открывшемся окне выбрать таблицы. После этого диаграмма будет актуализирована. 
...
\begin{enumerate}
	...
	\item Нажмите на кнопку \textit{Создать}
\end{enumerate}
...
\begin{enumerate}
	...
	\item Нажмите кнопку \begin{tikzpicture}
	\pgftext{\includegraphics[width=11pt]{img/DropTable16.png}} at (0pt,0pt)
	\end{tikzpicture} или клавишу \ttt{Delete}
\end{enumerate}
...
Диаграмма ERD позволяет выполнять несколько настроек, включая позиционирование каждой таблицы на холсте, выбор цвета фона и таблиц, выбор шрифта и стиля линий. Большинство этих функций доступны через панель инструментов, описанную \hyperref[sec:81]{выше}. Остальные доступны через всплывающее меню по нажатию правой кнопки мыши на холсте.

\anonsubsubsection{Добавление текстового поля}

На панель ERD можно добавить текстовое поле, нажав на кнопку \begin{tikzpicture}
	\pgftext{\includegraphics[width=11pt]{img/AddComment16.png}} at (0pt,0pt)
\end{tikzpicture} на панели инструментов.
У текстового поля можно изменить цвет фона, выделив его и нажав на кнопку на панели инструментов. Также можно менять шрифт. 

\anonsubsubsection{Перемещение объектов}

Панель ERD дает возможность перемещать выбранные таблицы по холсту для получения более четкого представления структуры базы данных. Чтобы переместить таблицу, щелкните левой кнопкой мыши по заголовку таблицы и перетащите ее в нужное место. Связи между таблицами останутся неизменными. Также можно выделить несколько таблиц. Для выделения можно использовать горячие клавиши \ttt{Ctrl + клик} и \ttt{Ctrl + A}. 
...
На диаграмме таблицы могут быть показаны со всеми полями или только теми, которые имеют такие ограничения, как первичный, внешний и уникальный ключи. Последний вариант может быть более предпочтительным в больших структурах базы данных, где пространство ограничено. Вид можно изменить через всплывающее меню (щелкнуть правой кнопкой мыши в области холста) \textit{Вид}$\rightarrow$\textit{Отображать только ключи базы данных}.

Для отображения комментариев к таблицам или полям откройте меню \textit{Вид}$\rightarrow$\textit{Отображать комментарии к таблицам/полям}.
...
\begin{longtable}[r]{|>{\ttfamily}m{5cm}|m{10.5cm}|}
	...
	Шаблон времени с учётом часового пояса & Шаблон времени для типа данных \ttt{TIME WITH TIME ZONE}. В таблице ниже рассмотрены подробно параметры шаблона\\\hline
	Шаблон даты-времени с учётом часового пояса & Шаблон даты-времени для типа данных \ttt{TIMESTAMP WITH TIME ZONE}. В таблице ниже рассмотрены подробно параметры шаблона\\\hline
