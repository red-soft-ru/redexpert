...
Описание значений столбцов вкладки \ttt{Таблицы}:
\begin{longtable}[r]{|>{\ttfamily}m{4.6cm}|m{10.9cm}|}
	\hline
	\centering\normalfont\bfseries Столбец &
	\centering\arraybslash\bfseries Описание\\\hline
	...
	table size(without blobs) & Размер таблицы в байтах. Вычисляемый столбец\\\hline
	table size + level0 & Размер таблицы в байтах, включая блобы уровня 0. Вычисляемый столбец\\\hline
	level1 + level2 & Размер блобов в байтах, которые не входят в уровень 0. Вычисляемый столбец\\\hline
	size with blobs & Совокупный размер данных таблицы (\ttt{table size(without blobs) + (table size + level0)}). Вычисляемый столбец\\\hline
	full size with indices & Совокупный размер данных таблицы (\ttt{size with blobs}) + совокупный размер всех индексов по таблице. Вычисляемый столбец\\\hline
...
Описание значений столбцов вкладки \ttt{Индексы}:
\begin{longtable}[r]{|>{\ttfamily}m{4.6cm}|m{10.9cm}|}
	\hline
	\centering\normalfont\bfseries Столбец &
	\centering\arraybslash\bfseries Описание\\\hline
	...
	real selectivity & Вычисляется по формуле \ttt{1 / (nodes - totalDup)}\\\hline
