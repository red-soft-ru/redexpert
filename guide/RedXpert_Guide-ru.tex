...
RedXpert (Ред Эксперт) - это GUI-оболочка, предоставляющая интерфейс для работы с базами данных Firebird и Ред Базой Данных. Написанная полностью на Java, RedXpert является программой, независимой от операционной системы, для которой требуется только JDK не ниже 1.8.

Используя гибкость, предоставляемую Java Database Connectivity (JDBC), RedXpert обеспечивает простой способ взаимодействия с базой данных от простых запросов до создания таблиц и импорта / экспорта данных всей базе данных.

RedXpert был протестирован под Windows, Linux с использованием Java 1.8.

RedXpert обладает рядом полезных компонентов: визуальным редактором для всех объектов базы данных, редактором SQL запросов и исполнителем SQL скриптов и др.

\newpage

\section{Установка RedXpert}

\begin{enumerate}[leftmargin=15pt]
	\item Установите JDK не ниже 1.8.
	\item Скачайте дистрибутив RedXpert для соответствующей операционной системы и ее разрядности с официального сайта СУБД Ред Базы Данных --- \href{http://reddatabase.ru/downloads/redexpert/}{reddatabase.ru}. Загрузка доступна только авторизованному пользователю.
	\item Запустите установку RedXpert с помощью файла \ttt{red\_expert\_installer-1.0.6-xxxxxx}. Инсталляция RedXpert осуществляется с помощью стандартного мастера установки программ.
...
\begin{itemize}[leftmargin=15pt]
	...
	\item Если для вашей системы не предусмотрен инсталятор, то можно запустить приложение командой "java -jar RedExpert.jar" из каталога в котором находится RedExpret.jar
\end{itemize}
\end{redremark}
\end{enumerate}	
...
\section{Обновление RedXpert}
Для того, чтобы не пропускать появление свежих версий RedXpert:
...
\item По окончании обновления перезапустите RedXpert.
\end{enumerate}	
...
В RedXpert используется распространенный для таких инструментов и даже большинства IDE интерфейс с вкладками и подключаемыми компонентами.
...
Предназначен для регистрации в RedXpert других JDBC драйверов помимо Jaybird 3 Driver, а также просмотра и редактирования их настроек.
	\begin{figure}[H]
		\centering
...
Можно заранее написать код из SQL команд во внешнем редакторе и сохранить его в файле операционной системы. RedXpert позволяет работать с такими файлами: считывать их в буфер, запускать на выполнение и сохранять набранные команды в файле.
...
RedXpert включает 6 основных групп панелей инструментов в верхней части окна приложения. Отдельные группы могут перемещаться горизонтально или вертикально вдоль верхней части окна приложения. Каждую группу инструментов можно скрыть или сделать видимой через меню \textit{Вид} или в настройках приложения.
...
RedXpert использует информацию, доступную из установленных JDBC драйверов базы данных, для генерации большей части ее содержимого.

По умолчанию в RedXpert уже установлена библиотека драйвера Jaybird 3 Driver, которая позволяет работать с базами данных Firebird и Ред Базой Данных.
...
RedXpert позволяет одновременно использовать несколько подключений к базе данных. Информация о подключении может быть получена через вкладку «Браузер базы данных».
...
Для тех, у кого уже есть настроенные в IBExpert подключения, разработана удобная опция импорта подключений в RedXpert.
...
\begin{enumerate}[leftmargin=15pt]
	...
	\item Далее запустите RedXpert и подключитесь к той пользовательской базе данных.
...
Выполнение запросов на добавление, удаление и обновление записей в таблице с помощью операторов \ttt{INSERT, DELETE} и \ttt{UPDATE} соответственно возможно в Редакторе запросов. Но в RedXpert есть возможность выполнить тоже самое не прибегая к написанию запросов.
...
Диалоговое окно \textit{Create user} позволяет создавать новых пользователей в выбранном соединении с базой данных.

\begin{enumerate}[leftmargin=26pt]
	\item На панели \textit{Подключения} щелкните правой кнопкой мыши по объекту ПОЛЬЗОВАТЕЛИ и выберите пункт \textit{Создать USER} всплывающего меню. Откроется диалоговое окно.
	\begin{figure}[H]
		\centering
...
На боковой панели \textit{Подключения} щелкните правой кнопкой мыши по имени пользователя и выберите пункт всплывающего меню \textit{Редактировать <имя роли>:USER}. откроется окно редактирования пользователя

\anonsubsubsection{Удаление пользователей}
На боковой панели \textit{Подключения} щелкните правой кнопкой мыши по имени пользователя и выберите пункт всплывающего меню \textit{Удалить <имя пользователя>:USER}. Откроется  диалоговое окно для подтверждения или отката операции удаления пользователя (см. \hyperref[ris:01]{рис.~\ref{ris:01}}). Для подтверждения операции нажмите кнопку «Подтвердить».
...
В RedXpert есть возможность сравнить типы данных разных СУБД. Задача состоит в том, чтобы найти аналогичные типы данных, которые наиболее точно соответствуют друг другу. Это часто требуется при переносе базы данных из одной СУБД в другую.
...
\begin{enumerate}
	\item На главной панели меню RedXpert выберите пункт \textit{Файл}$\rightarrow$\textit{Сохранить} или выберите кнопку
	\begin{tikzpicture}
	\pgftext{\includegraphics[width=11pt]{img/save16.png}} at (0pt,0pt)
	\end{tikzpicture} 
	на главной панели инструментов
...
Ключевые слова, добавленные пользователем RedXpert, распознаются в редакторе запросов, подсвечиваются и присутствуют в всплывающей подсказке (по нажатию \ttt{Ctrl+Space}).
...
Системный журнал вывода хранится в \ttt{\$HOME/.redexpert/logs} и его можно открыть любым текстовым редактором. Но его также можно посмотреть и с помощью RedXpert, выбрав пункт меню  \textit{Инструменты}$\rightarrow$\textit{Системные журналы состояния}$\rightarrow$\textit{Системный журнал состояния вывода}. На центральной панели будет открыта новая вкладка с содержимым журнала.
...
\begin{redremark}
	Большинство изменений в настройках вступают в силу после перезапуска RedXpert.
\end{redremark}
...
Здесь перечислены действующие горячие клавиши для различных команд при работе с RedXpert. Их можно изменить, удалить или добавить новые. Для этого просто щелкните мышью по нужной ячейке -- откроется диалоговое окно:
\begin{figure}[H]
	\centering
...
\begin{longtable}[r]{|>{\ttfamily}m{5cm}|m{10.5cm}|}
	...
	\endhead
	\hline
	Автоматически подключаться при запуске & Подключаться к выбранной в пункте \textit{Подключение для автозапуска} базе данных при запуске RedXpert\\\hline
	Подключение для автозапуска &  Выберите подключение, которое будет установлено при запуске RedXpert\\\hline
\end{longtable}
