...
public:
    ...
    // IUnknown methods.
    // Note that IE never calls any of these methods, since the caller owns
    // the IBindStatusCallback interface, so the methods all return zero/E_NOTIMPL.
...
SharedLibraryHandle openSharedLibrary(
    const std::string &sl_file,
    std::string path,
    bool from_file_java_paths)
{
...
public:
    JvmRegistryKey(
        ...
        const std::string &a_jre_path) : m_hive(hive),
                                         ver(a_ver),
...
SharedLibraryHandle tryPaths()
{
    ...
    std::ifstream in(path_to_java_paths); // открываем файл для чтения
    if (in.is_open())
    {
...
struct JavaVendorRegistryLocation
{
    ...
    JavaVendorRegistryLocation(
        ...
        const char *a_jre_name) : java_vendor(a_java_vendor),
                                  jdk_name(a_jdk_name),
                                  jre_name(a_jre_name)
    {
    }
...
int try_dlopen(std::vector<std::string> potential_paths, void *&out_handle, bool from_file_java_paths)
{
    ...
    for (std::vector<std::string>::iterator it = potential_paths.begin(), en = potential_paths.end(); it != en; ++it)
    {
        ...
        if ((out_handle != 0) && (!from_file_java_paths))
        {
            ...
            std::ofstream file_java_paths;            // поток для записи
            file_java_paths.open(path_to_java_paths); // открываем файл для записи
...
// Hopefully our caller will report something more useful than the shared library handle.
    ...
    jclass findClass(const std::string &c_name)
    {
        ...
        if (j_class == 0)
        {
            ...
            os << "FindClass(\"" << can_name << "\") failed (FindClass loads and initializes the class as well as finding it).";
...
// if linux system is used need to set desktop environment to NONE, otherwise launcher is crashed
#ifdef __linux__
