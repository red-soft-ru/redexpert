lexer  grammar SqlLexer;
 ...
 : (SMALLINT | INTEGER | BIGINT) ARRAY_SIZE?
    ...
    | (CHAR | CHARACTER | VARYING_CHARACTER | VARCHAR) ('('INT_NUMBER')')?
    (CHARACTER_SET IDENTIFIER)? ARRAY_SIZE?
    | (NCHAR | NATIONAL_CHARACTER | NATIONAL_CHAR) (VARYING)? ('(' DIGIT+ ')')? ARRAY_SIZE?
    | BLOB (SUB_TYPE (IDENTIFIER | INT_NUMBER))?
    (SEGMENT_SIZE INT_NUMBER)? (CHARACTER_SET IDENTIFIER)?
    | BLOB ('('INT_NUMBER (',' INT_NUMBER)?')')?
 ;
